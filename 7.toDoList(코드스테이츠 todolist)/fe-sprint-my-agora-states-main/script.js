...
let localStorageObj = localStorage.getItem("obj"); // localstorage data를 불러오는 변수 생성
if (localStorageObj) {
  // localstorage에 저장된 값이 존재한다면 if문 실행
  localStorageObj = JSON.parse(localStorage.getItem("obj")); // JSON.parse()를 통해 localstorage에 text로 저장된 값을 js의 데아터 타입으로 변환(역직렬화)하고 변수값을 갱신
} else {
